...
const addPredefinedSpecialBtn = $('#add-predefined-special-btn');
const predefinedSpecialSelect = $('#predefined-special-select');
const specialPlayerInputGroup = $('#special-player-input-group');
const specialThresholdInputGroup = $('#special-threshold-input-group');
...
let calculatedProbs = {}; // To store calculated probabilities for combos
...
const scoreAndWin = (muTeam, muOpponent, playerLambda, maxGoals = 10) => {
    if(muTeam <= 0) return 0;
    let s = playerLambda / muTeam; // Udeo igrača u golovima tima
    let prob = 0;
    
    for (let gTeam = 1; gTeam <= maxGoals; gTeam++) {
        let pTeam = poissonPMF(muTeam, gTeam);
        for (let gOpponent = 0; gOpponent < gTeam; gOpponent++) {
...
const calculatePlayerOdds = () => {
    let allResults = [];
    calculatedProbs = {}; // Reset probabilities
    ...
    playerCards.forEach(card => {
        ...
        if (!calculatedProbs[playerName]) calculatedProbs[playerName] = {};
        ...
        if (baseOdds['daje-gol'] > 0) {
            ...
            calculatedProbs[playerName]['daje-gol'] = oddToProb(baseOdds['daje-gol']);
            ...
            calculatedProbs[playerName]['daje-2-plus-golova'] = oddToProb(odd_2plus_final);
            ...
            calculatedProbs[playerName]['daje-3-plus-golova'] = oddToProb(odd_3plus_final);
        ...
        if (baseOdds['sutevi-okvir-1'] > 0) {
            ...
            calculatedProbs[playerName]['sutevi-okvir-1'] = oddToProb(baseOdds['sutevi-okvir-1']);
...
const calculateSpecialOdds = () => {
    const results = [];
    calculatedProbs['match'] = {}; // Reset match probabilities
    const margin = parseFloat($('#bookmaker-margin').value) || 0;
    ...
    const createSpecialBet = (m2, m3, prob, key) => {
        if (prob <= 0 || prob >= 1) return;
        if (key) calculatedProbs['match'][key] = prob;
    ...
    // Calculations
    createSpecialBet('Oba tima 2+ kartona', '', p_2plus_cards_team * p_2plus_cards_team, 'both-teams-2-plus-cards');
    ...
    createSpecialBet('3+ golova', 'i 10+ kornera', p_3plus_goals * p_10plus_corners, '3-plus-goals-10-plus-corners');
    createSpecialBet('3+ golova', 'i 4+ kartona', p_3plus_goals * p_4plus_cards, '3-plus-goals-4-plus-cards');
    ...
    createSpecialBet('Izmena postize gol', '', 1/(goalsLambda*0.15));
    createSpecialBet('Precka ili stativa', 'na mecu', 0.5);
    createSpecialBet('Gol iz slobodnog udarca', '', 1/(goalsLambda*0.04));
    createSpecialBet('Gol izvan 16m', '', 1/(goalsLambda*0.11));
    createSpecialBet('Dosudjen penal', 'za oba tima', (p_penal/2) * (p_penal/2));
    createSpecialBet('Gol glavom na mecu', '', 1/(goalsLambda*0.17));
...
const fetchApiEvents = async () => {
    ...
    try {
        ...
        const competitionsWithRelevantMarkets = data.competitions.map(comp => {
            const relevantEvents = comp.events.filter(event => event.markets && (event.markets['soccer.anytime_goalscorer'] || event.markets['soccer.totals.corners']));
            return { ...comp, events: relevantEvents };
        }).filter(comp => comp.events.length > 0);

        apiEvents = competitionsWithRelevantMarkets.flatMap(comp => comp.events.map(event => ({...event, competitionName: comp.name, competitionKey: comp.key })));

        if (apiEvents.length === 0) {
             apiStatus.textContent = 'Nije pronađen nijedan meč sa relevantnim kvotama (strelci, korneri).';
        } else {
             populateCompetitionSelect(competitionsWithRelevantMarkets);
...
const populateApiPlayerSelect = (players) => {
    ...
    $('#special-player-select').innerHTML = '<option value="">-- Izaberi igrača --</option>';
    ...
    players.sort((a, b) => a.goalscorerOdd - b.goalscorerOdd).forEach(player => {
        ...
        apiPlayerSelect.appendChild(option.cloneNode(true));
        $('#special-player-select').appendChild(option);
    });
...
tabButtons.forEach(button => {
    on(button, 'click', () => {
        ...
        $('#generate-btn-text').textContent = tab === 'players' ? 'Generiši Kvote' : 'Generiši Osnovne Specijale';
        previewSection.classList.add('hidden');
...
on(generateBtn, 'click', (e) => { 
    ...
    setTimeout(() => {
        let generatedData = [];
        if (currentTab === 'players') {
            generatedData = calculatePlayerOdds();
        } else {
            generatedData = calculateSpecialOdds();
        }
        updatePreviewTable(generatedData);
        
        btnText.textContent = currentTab === 'players' ? 'Generiši Kvote' : 'Generiši Osnovne Specijale';
...
// --- Predefined Specials Logic ---
// ... (Ovaj deo će biti dodat)
